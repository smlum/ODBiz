# Messy version for fire departments
...
input_data = "Canada Fire Depts Rec Type 2-3 12.csv"
...
df = df[~df['street_address'].isnull()]

# Filter out PO Box addresses
df = df[~df['street_address'].str.contains('box', case=False)]

# Filter out those without a number
df = df[df['street_address'].str.contains(r'\d')]

print('Length after filtering: ', len(df))


# filter to those with a street number and street name
# df = df[~df.street_address.isna()]
...
df['gc_request_street'] = df['street_address'] + ", " + df['city_town']
...
json_name = 'fire_departments.json'
